...
**脚注**
<a href="#[1]"[1]></a>
<a name="[1]">[1]</href> 由于`**`的优先级高于` -` ，`-3** 2` 将被解释为 `-（3**2）`，从而导致结果为 -9。为了避免这种情况，并得到 9，你可以使用`（-3）** 2`

<a name="[2]"></a>[2](#[2]) 与其他语言不同,特殊字符如`\ n`，具有相同含义的两个单引号(`'…'`)和双引号(`“…”`)。两者之间唯一的区别是在单引号不需要转码建 `”`(但你必须有转码键`\ '`)，反之亦然。
